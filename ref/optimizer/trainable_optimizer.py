...
training_output = collections.namedtuple(
    "TrainingOutput", [
        "metaobj",
        "obj_weights",
        "problem_objectives",
...
update_results = collections.namedtuple(
    "UpdateResults", [
        "new_params",
        "new_states",
        "global_state",
...
class TrainableOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def __init__(
            self, name, state_keys, use_attention=False,
        ...
        Args:
            ...
            use_attention: Whether this optimizer uses attention
                (Default: True)
            use_log_objective: Whether this optimizer uses the logarithm of the
                objective when computing the loss (Default: False)
            obj_train_max_multiplier: The maximum multiplier for the increase
                in the objective before meta-training is stopped. If <= 0,
                meta-training is not stopped early. (Default: -1)
            use_second_derivatives:
                Whether this optimizer uses second derivatives in
                meta-training. This should be set to False if some second
                derivatives in the meta-training problem set are not defined in
                Tensorflow. (Default: True)
            use_numerator_epsilon: Whether to use epsilon in the numerator when
                scaling the problem objective during meta-training.
                (Default: False)
    ...
    def _initialize_state(self, var):
        ...
        Returns:
            state: a dictionary mapping state keys to initial state values
            (tensors)
        """
        return {}
    ...
    def _apply_common(self, grad, var):
        ...
        Returns:
            update_op: A tensorflow op that assigns new values to the variable,
                and also defines dependencies that update the state variables
                for the optimizer.
        """
    ...
    def _compute_update(self, param, grad, state):
        ...
        Args:
            ...
            grad: The gradient tensor of the objective with respect to the
                parameters. (It has the same shape as param.)
            state: A dictionary containing any extra state required by the
                optimizer.
    ...
    def _compute_updates(self, params, grads, states, global_state):
        ...
        Returns:
            ...
            attention_params: A list of attention parameters. This is the same
                as new_params if the optimizer does not use attention.
        """
        ...
        return update_results(
            ...
            attention_params=list(new_params)
        )
    ...
    def train(self, problem, dataset):
        ...
        Returns:
            meta_objective: A tensorflow operation for computing the
                meta-objective
            obj_weights: A tensor placeholder for feeding in the objective
                weights
            ...
            reset_state: A placeholder signifying that the rnn state should be
                reset.
        ...
        data, labels = dataset
        # Convert the ndarrays to tensors so we can pass them back in via
        # feed_dict
        ...
        def loop_body(
                itr, obj_accum, params, attend_params, flattened_states,
                global_state, all_obj, unused_init_obj, data,
                labels, batches):
            """Body of the meta-training while loop for optimizing a
            sub-problem.

            Args:
                itr: The current meta-training iteration.
                obj_accum: The accumulated objective over all training steps so
                    far.
                params: The parameters of the sub-problem.
                attend_params: The parameters of the sub-problems at the
                    attended location.
                flattened_states: The states of the trainable optimizer, sorted
                    and flattened into a list (since a while loop can't handle
                    nested lists or dictionaries).
                global_state: The global state of the optimizer.
                all_obj: The list of all objective values in the training
                    process.
                unused_init_obj: The initial objective (unused here, but needed
                    in the variable list because it's used in a stopping
                    condition in the loop_cond.)
                data: The data for this problem.
                labels: The labels corresponding to the data.
                batches: The batch indexes needed for shuffled minibatch
                    creation.
            ...
            Returns:
                ...
                attend_params: The new parameters of the sub-problems at the
                    attended location.
            ...
            _p = attend_params if self.use_attention else params
            current_obj = problem.objective(_p, batch_data, batch_labels)
            grads = problem.gradients(current_obj, _p)

            if not self.use_second_derivatives:
                grads = [
                    tf.IndexedSlices(
                        tf.stop_gradient(grad.values), grad.indices)
                    if isinstance(grad, tf.IndexedSlices)
                    else tf.stop_gradient(grad)
                    for grad in grads
                ]

            # store the objective value for the entire problem at each
            # iteration
            ...
            # Set the shape to keep the shape invariant for obj_accum. Without
            # this, the graph builder thinks the tensor shape is unknown on the
            # 2nd iter.
            obj_accum.set_shape([])

            # convert flattened_states to dictionaries
            dict_states = [
                dict(zip(self.state_keys, flat_state))
                for flat_state in flattened_states
            ]

            # compute the new parameters and states
            updates = self._compute_updates(
                params, grads, dict_states, global_state)

            # flatten the states
            new_flattened_states = list(
                map(flatten_and_sort, updates.new_states))

            return [
                itr + 1, obj_accum, updates.new_params,
                updates.attention_params, new_flattened_states,
                updates.global_state, all_obj,
                unused_init_obj, data, labels, batches
            ]

        def loop_cond(
                itr, obj_accum, unused_params, unused_attend_params,
                unused_flattened_states, unused_global_state, all_obj,
                init_obj, *args):
            """Termination conditions of the sub-problem optimization loop."""
            del args  # unused

            # We've run < num_iter times
            cond1 = tf.less(itr, num_iter)
            # The objective is still finite
            cond2 = tf.math.is_finite(obj_accum)
            ...
            if self.obj_train_max_multiplier > 0:
                ...
                # The objective is a reasonable multiplier of the original
                cond3 = tf.less(current_obj, max_obj)

                return tf.logical_and(
                    tf.logical_and(cond1, cond2), cond3,
                    name="training_loop_cond")
            else:
                return tf.logical_and(cond1, cond2, name="training_loop_cond")

        (
            loop_vars, invariants, initial_obj, init_loop_vars_to_override
        ) = self._initialize_training_loop_parameters(
            problem, data, labels, batches, first_unroll, reset_state)

        loop_output = tf.while_loop(
            loop_cond, loop_body, loop_vars,
            swap_memory=True, shape_invariants=invariants)
        meta_obj, problem_objectives = loop_output[1], loop_output[6]

        # The meta objective is normalized by the initial objective at the
        # start of the series of partial unrolls.
    ...
    def _initialize_training_loop_parameters(
        ...
        Args:
            ...
            first_unroll: Whether this is the first unroll in a partial
                unrolling.
            reset_state: Whether RNN state variables should be reset.

        Returns:
            loop_vars: The while loop variables for training.
            invariants: The corresponding variable shapes (required by while
                loop).
            initial_obj: The initial objective (used later for scaling).
            init_loop_vars_to_override: The loop vars that can be overridden
                when performing training via partial unrolls.
        """
    ...
    def scale_objective(
            self, total_obj, all_objs, initial_obj, obj_scale_eps=1e-6):
        ...
        Args:
            ...
            all_objs: A list of all the individual objectives over the training
                run.
            initial_obj: The initial objective value.
            obj_scale_eps: The epsilon value to use in computations for
                stability.
        ...
        if self.use_log_objective:
            if self.use_numerator_epsilon:
                ...
                log_scaled_problem_obj = tf.log(
                    scaled_problem_obj + obj_scale_eps)
...
def local_state_variables(init_values, return_init_values):
    ...
    variable_use_count = tf.compat.v1.get_collection_ref(
    ...
    with tf.compat.v1.variable_scope(OPTIMIZER_SCOPE):
        ...
        for init_value in init_values:
            ...
            name = (
                _LOCAL_VARIABLE_PREFIX
                + "_".join(map(str, init_value.get_shape().as_list()))
                + "_" + init_value.dtype.name
            )
            unique_name = name + "_" + str(variable_use_count[name])
            variable_use_count[name] += 1
            # The overarching idea here is to be able to reuse variables
            # between different sessions on the same TensorFlow master without
            # errors. By uniquifying based on the type and name we mirror the
            # checks made inside TensorFlow, while still allowing some memory
            # reuse. Ultimately this is a hack due to the broken
            # Session.reset().
            local_vars.append(
