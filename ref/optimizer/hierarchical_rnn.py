...
class HierarchicalRNN(opt.TrainableOptimizer):
  ...
  def __init__(self, level_sizes, init_lr_range=(1e-6, 1e-2),
    ...
    with tf.compat.v1.variable_scope(opt.OPTIMIZER_SCOPE):
      ...
      self.update_weights = tf.compat.v1.get_variable("update_weights",
                                            shape=(cell_size, 1),
      ...
      if self.use_attention:
        ...
        self.attention_weights = tf.compat.v1.get_variable(
            "attention_weights",
      ...
      with tf.compat.v1.variable_scope(scope):
        ...
        if self.learnable_rnn_init:
          self.init_vectors.append(tf.Variable(
              tf.random.uniform([1, hcell.state_size], -1., 1.,
                                seed=self.random_seed),
              name="init_vector"))
        else:
  ...
  def _initialize_scale_decay(self, weights_tensor_shape, scaled_init):
    ...
    if self.learnable_decay:
      self.scl_decay_weights = tf.compat.v1.get_variable("scl_decay_weights",
                                               shape=weights_tensor_shape,
                                               initializer=scaled_init)
      scl_decay_bias_init = tf.constant_initializer(
          FLAGS.scale_decay_bias_init)
      self.scl_decay_bias = tf.compat.v1.get_variable("scl_decay_bias",
                                            shape=(1,),
  ...
  def _initialize_input_decay(self, weights_tensor_shape, scaled_init):
    ...
    if (self.learnable_decay and self.num_gradient_scales > 1 and
      ...
      self.inp_decay_weights = tf.compat.v1.get_variable("inp_decay_weights",
                                               shape=weights_tensor_shape,
                                               initializer=scaled_init)
      inp_decay_bias_init = tf.constant_initializer(
          FLAGS.hrnn_default_decay_var_init)
      self.inp_decay_bias = tf.compat.v1.get_variable("inp_decay_bias",
                                            shape=(1,),
  ...
  def _initialize_lr(self, weights_tensor_shape, scaled_init):
    ...
    if self.dynamic_output_scale:
      ...
      self.lr_weights = tf.compat.v1.get_variable("learning_rate_weights",
                                        shape=weights_tensor_shape,
                                        initializer=wt_init)
      self.lr_bias = tf.compat.v1.get_variable("learning_rate_bias", shape=(1,),
  ...
  def _initialize_state(self, var):
    ...
    if self.dynamic_output_scale:
      ...
      if min_lr == max_lr:
        ...
        actual_vals = tf.random.uniform(var_vectorized.get_shape().as_list(),
                                        np.log(min_lr) / 2.,
                                        np.log(max_lr) / 2.,
                                        seed=self.random_seed)
        offset = tf.random.uniform((), np.log(min_lr) / 2., np.log(max_lr) / 2.,
                                   seed=self.random_seed)
  ...
  def _compute_updates(self, params, grads, states, global_state):
    ...
    with tf.compat.v1.variable_scope(opt.OPTIMIZER_SCOPE):
      ...
      for param, grad_unflat, state in zip(params, grads, states):
        ...
        with tf.compat.v1.variable_scope("PerTensor", reuse=self.reuse_vars):
      ...
      updated_global_state = self._compute_updated_global_state(
        [layer_state], global_state)

    return opt.update_results(
        new_params=updated_params,
        new_states=updated_states,
        global_state=[updated_global_state],
        attention_params=updated_attention)
  ...
  def _extend_rnn_input(self, rnn_input, state, grads_scaled,
    ...
    if self.use_log_means_squared:
      log_means_squared = [tf.math.log(ms + 1e-16)
    ...
    if self.use_relative_lr or self.use_extreme_indicator:
      ...
      if self.use_extreme_indicator:
        ...
        extreme_indicator = (tf.nn.relu(log_lr_vec - tf.math.log(1.)) -
                             tf.nn.relu(tf.math.log(1e-6) - log_lr_vec))
        rnn_input.append(extreme_indicator)

    if self.use_lr_shortcut:
      log_lr_vec = tf.reshape(state["log_learning_rate"], [-1, 1])
      rnn_input.append(log_lr_vec - tf.math.log(1e-3))
  ...
  def _update_rnn_cells(self, state, global_state, rnn_input_tensor,
    ...
    with tf.compat.v1.variable_scope("Layer0_RNN"):
      total_bias = None
      if self.num_layers > 1:
  ...
  def _compute_new_learning_rate(self, state, new_param_state):
    if self.dynamic_output_scale:
      ...
      lr_momentum_logit = tf.compat.v1.get_variable(
      ...
      new_log_lr = (lrm * state["log_learning_rate"] +
                    (1. - lrm) * step_log_lr)
      param_stepsize_offset = tf.compat.v1.get_variable("param_stepsize_offset",
                                              initializer=-1.)
  ...
  def _compute_updated_global_state(self, layer_states, global_state):
    ...
    if self.num_layers == 3:
      ...
      #   bias -> None
      with tf.compat.v1.variable_scope("Layer2_RNN", reuse=self.reuse_global_state):
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    with tf.op_scope([], name, self._name) as name:
      ...
      with tf.compat.v1.variable_scope(self._name, reuse=self.reuse_global_state):
        gs = self._initialize_global_state()
        if gs:
          global_state = [tf.compat.v1.get_variable("global_state", initializer=gs[0])]
        else:
          global_state = []
