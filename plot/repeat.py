...
class RepeatResults:
    ...
    def _get_default_period(self, t, period=-1, repeat=-1):
        """Get default period and repeat indices."""
        if period != -1 and repeat != -1:
    ...
    def get_eval(self, t, period=-1, repeat=-1, problem="conv_train"):
        ...
        if t in self.baselines:
            ...
            if key not in self._test_cache:
                period, repeat = self._get_default_period(t, period, repeat)
                fn = "period_{}.{}.npz".format(period, repeat)
                self._test_cache[key] = np.load(
                    os.path.join(self.dir_results, t, "eval", problem, fn))
            return self._test_cache[key]

    def get_train_log(self, t, period=-1, repeat=-1):
        """Get log file for a single period."""
        period, repeat = self._get_default_period(t, period, repeat)
        return np.load(os.path.join(
            self.dir_results, self.results[t], "log",
            "period_{}.{}.npz".format(period, repeat)))
