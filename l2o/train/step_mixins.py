...
class StepMixin:
    ...
    def _summarize(self, meta_loss, imitation_loss, state, callback_states):
    ...
    def abstract_train_step(
        ...
        def _inner(data, state):
            ...
            with tf.GradientTape() as tape:
                results = self.abstract_loss(data, state, **kwargs)
                loss = (
        ...
        return self._summarize(
            *distribute.run(_inner, args=(data, state)))

    @tf.function
    def abstract_valid_step(self, data, state, **kwargs):
        """Single outer step (validation mode)."""

        def _inner(data, state):
            return self.abstract_loss(data, state, **kwargs)

        distribute = tf.distribute.get_strategy()
        return self._summarize(
            *distribute.run(_inner, args=(data, state)))

    def make_concrete_step(self, meta, data, state):
        ...
        if meta.validation:
            step = self.abstract_valid_step.get_concrete_function(
                data, state, unroll=meta.unroll_len, problem=meta.problem,
                seed=meta.seed)
        else:
