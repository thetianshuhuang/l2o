  ...
  with tf.compat.v1.variable_scope(scope):
    ...
    if include_bias:
      bias = tf.compat.v1.get_variable(
        ...
        initializer=tf.constant_initializer(bias_init, tf.float32))
    else:
      bias = 0.
...
def new_mean_squared(grad_vec, decay, ms):
  ...
  decay_check_ops = [
      tf.compat.v1.assert_less_equal(decay, 1., summarize=decay_size),
      tf.compat.v1.assert_greater_equal(decay, 0., summarize=decay_size)]
...
def rms_scaling(gradient, decay, ms, update_ms=True):
  ...
  scaled_gradient = tf.math.asinh(grad_vec / tf.sqrt(ms + 1e-16))
